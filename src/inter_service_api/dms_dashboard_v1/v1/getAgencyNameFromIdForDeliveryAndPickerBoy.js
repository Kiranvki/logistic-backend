...
module.exports = async (cityId, agencyId) => {
  try {
    ...
    info(`Hitting the DMS V1 server to get the Delivery Executive and PickerBoy agency name from ID!`);

    // getting the data from the env
    let dmsV1BaseUrl = process.env.dmsV1BaseUrl;
    let dmsGetAgencyName = process.env.dmsGetAgencyNamePartOne + agencyId + process.env.dmsGetAgencyNamePartTwo + cityId;
    let url = dmsV1BaseUrl + dmsGetAgencyName; // DMS url
    console.log('url', url);
