...
module.exports = async (req, res, next) => {
  try {
    ...
    for (let i = 0; i < req.body.length; i++) {
      ...
      // get the portal type 
      let portalType = req.body[i].portalType || 'admin';

      // get the firstname
      req.body[i].firstName = req.body[i].firstName.replace(
        /\w\S*/g,
        function (txt) {
      ...
      req.body[i].lastName = req.body[i].lastName.replace(
        /\w\S*/g,
        function (txt) {
          return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase();
        });
      ...
      let fullName = `${req.body[i].firstName} ${req.body[i].lastName}`;

      let email = req.body[i].email,
        passwordToEmail = req.body[i].actualPassword;
      ...
      compiledHTML = await pug.compileFile(templatePath)({
        ...
        url: process.env.frontEndHost + process.env.frontEndPort,
        name: fullName,
        email: email,
