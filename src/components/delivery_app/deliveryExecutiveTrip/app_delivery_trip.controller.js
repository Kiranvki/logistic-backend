...
class DeliveryExecutivetrip extends BaseController {
 ...
 //   {$unwind: {path: "$salesorders", preserveNullAndEmptyArrays: false}},
    {$project:{tripId:1, vehicleRegNumber:1,
  ...
  getOrderByCustomer = async (req, res, next) => {
    ...
    let customerSalesOrderData = await tripSalesOrders.find({ '_id': mongoose.Types.ObjectId(ID) })
      .populate({
        path: 'salesOrderId', 'match': { 'customerPhone': phNo }, options: {
          limit: pageSize,
          sort: { created: -1 },
  ...
  getInvoiceByNumber = async (req, res, next) => {
    ...
    let pipeline = [
      ...
      {
        $group: {_id: "$invoiceDetails.invoiceNo",
          so_db_id: {$first:"$so_db_id"}, "orderPlacedAt": {$first:"$createdAt"}, "invoiceNo": {$first:"$invoiceDetails.invoiceNo"}, 
          item:{$push:{"Id": "$salesorders.orderItems._id", "itemId": "$salesorders.orderItems.material_no", 
          "suppliedQty": "$salesorders.orderItems.suppliedQty", "quantity": "$salesorders.orderItems.quantity",
          "itemName": "$salesorders.orderItems.material_description","itemRemarks": "$salesorders.orderItems.itemRemarks"}}
        }},
      // {
  ...
  getInTripInvoicelist = async (req, res, next) => {
    try {
      ...
      let invoiceNumber = await invoiceMasterModel.aggregate([
        ...
        {
          $project: {
            customerName: 1, "invoice": "$invoiceDetails.invoiceNo", "salesOrderNo": "$soId",
  ...
  getInvoiceVew = async (req, res, next) => {
    ...
    let pipeline = [
      ...
      {
        $project: {
          so_db_id: 1,isDelivered:1, "orderPlacedAt": "$createdAt", "invoiceNo": "$invoiceDetails.invoiceNo", "itemSupplied.itemId": 1, "itemSupplied._id": 1,"itemSupplied.suppliedQty": 1, "itemSupplied.quantity": 1,
  ...
  getHistoryByOrderType = async (req, res, next) => {
    ...
    if (type === 'salesorders' || type === 'salesOrder') {
      pipeline.push(
        ...[
          ...
          {
            $group: {
              _id: "$tripId", vehicleRegNumber: { $first: "$vehicleRegNumber" }, isDelivered: { $first: "$salesorder.invoices.isDelivered" }, so_db_id: { $first: "$salesOrder" },
    ...
    let totalCount = await tripModel.count({
      $and: [{ 'deliveryExecutiveId': deliveryExecutiveId },
      {
        'isActive': 0
      },
      {
  ...
  updateItemStatusAndCaretOut = async (req, res, next) => {
    try {
      ...
      orderItems.forEach(async (item, index) => {
        ...
        updatedOrderDetail = await salesOrderModel.updateOne({_id:mongoose.Types.ObjectId(_id),"orderItems":{$elemMatch:{'_id':mongoose.Types.ObjectId(item.id)}}},
        {$set:{
          'orderItems.$.itemDeliveryStatus': item.itemdeliverystatus ? item.itemdeliverystatus : 0,
          'orderItems.$.rejectedQuantity': item.rejectedquantity ? item.rejectedquantity : 0,
          'orderItems.$.comments': item.comments ? item.comments : ''
  ...
  getTripHistoryByDeliveryExecutiveId = async (req, res, next) => {
    ...
    { $project: { salesOrder: 1, vehicleRegNumber: 1, tripId: 1,createdAt:1 } },
    {
      $lookup: {
        from: "salesorders",
        let: { id: "$salesOrder" },
    ...
    {
      $project: {
        tripId: 1, vehicleRegNumber: 1,createdAt:1, "NoOfCrates": "$salesorders.crateIn", salesorders: { $cond: { if: { $isArray: "$salesorders.item" }, then: { $size: "$salesorders.item" }, else: "NA" } }
      }
    },
  ...
  getSalesOrdersbyTripID = async (req, res, next) => {
    try {
      ...
      let salesOrderlist = await tripModel.aggregate([
        ...
        {
          $project: {
            salesOrder: 1, vehicleRegNumber: 1, sold_to_party_description: "$salesorders.sold_to_party_description", sold_to_party: "$salesorders.sold_to_party",
      ...
      let invoiceNumber = await invoiceMasterModel.aggregate([
        ...
        { $unwind: { path: "$invoice" } },
        {
          $group: {
            ...
            invoices: {
              $push: {
                "invoice": "$invoiceDetails.invoiceNo", "orderPlacedAt": "$createdAt", "address1": "$shippingDetails.address1", "city": "$shippingDetails.cityId",
  ...
  viewDisputeDetails = async (req, res, next) => {
    ...
    let trip = await disputeModel.aggregate([
      ...
      {$project:
          {tripId:1, status:1,disputeId:1,dispute_amount:1,
         ...
         "deliveryExecutiveName":{$first:"$trips.deliveryExecutiveName"},
          ...
          "disputeDate":"$createdAt","truck_Number":{$first:"$trips.vehicleRegNumber"},
