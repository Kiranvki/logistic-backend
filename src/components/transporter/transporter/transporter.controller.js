...
const transporterMappingCtrl = require("../transporter_vehicle_mapping/transporter_vehicle_mapping.controller");
...
class transporterController extends BaseController {
    ...
    try {
      ...
      if (isInserted && !_.isEmpty(isInserted)) {
        return this.success(
          ...
          this.status.HTTP_OK,
          isInserted,
          this.messageTypes.transporterCreated
        );
      } else
        return this.errors(
          req,
          ...
          this.messageTypes.transporterNotCreated
        );
  ...
  getList = async (req, res) => {
    try {
      ...
      let transporterList = await Model.aggregate([
        ...
        {
          $unwind: {
            path: '$transporterVehicleMapping',
            preserveNullAndEmptyArrays: true
          }
        },
        {
          $project: {
            ...
            "transporterVehicleMapping.rateCategory._id":1,
            "transporterVehicleMapping.rateCategory.rateCategoryDetails.rateCategoryName": 1
          },
        },
      ]).allowDiskUse(true);
      // success
  ...
  //     {
  //       $lookup: {
  //         from: 'transporterratecategoryvehiclemappings',
  //         let: {
  //           'id': '$_id'
  //         },
  //         pipeline: [
  ...
  getTransporter = async (req, res) => {
    try {
      ...
      // check if inserted
      if (transporter && !_.isEmpty(transporter))
        return this.success(
          req,
          res,
          this.status.HTTP_OK,
        ...
        return this.errors(
          ...
          res,
          this.status.HTTP_CONFLICT,
          this.messageTypes.transporterNotFetched
        );
    ...
    try {
      ...
      if (isUpdated && !_.isEmpty(isUpdated))
        return this.success(
          ...
          this.status.HTTP_OK,
          isUpdated,
          this.messageTypes.transporterUpdated
        );
      else
        return this.errors(
          req,
          res,
          this.status.HTTP_CONFLICT,
          this.messageTypes.transporterNotUpdated
        );
    ...
    try {
      ...
      if (isUpdated && !_.isEmpty(isUpdated))
        return this.success(
          ...
          this.status.HTTP_OK,
          {},
          this.messageTypes.transporterDeleted
        );
      else
        return this.errors(
          req,
          res,
          this.status.HTTP_CONFLICT,
          this.messageTypes.transporterNotDeleted
        );
    ...
    try {
      ...
      if (isUpdated && !_.isEmpty(isUpdated))
        return this.success(
          ...
          isUpdated,
          type == "activate"
            ? this.messageTypes.transporterActivatedSuccessfully
            : this.messageTypes.transporterDeactivatedSuccessfully
        );
      else
        return this.errors(
          req,
