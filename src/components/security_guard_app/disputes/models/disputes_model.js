...
let disputeSchema = Schema(
  {
    tripId: { type: Number, unique: true, required: true },
    disputeId: { type: Number, unique: true, required: true },
    dispute_amount: { type: Number },
    salesOrderId: [{ type: mongoose.Schema.Types.ObjectId, ref: "salesOrder" }],
    invoiceId: [{ type: mongoose.Schema.Types.ObjectId, ref: "invoiceMaster" }],
    // 'invoiceNo': {//invoice_no // type: 'String', // },
